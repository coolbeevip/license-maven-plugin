...
public class ParseMvnRepository implements Parse {
  ...
  public LicensesRepository parseLicense(Optional<Log> log, String groupId, String artifactId,
      String version) {
    ...
    try {
      ...
      String homePage = doc.select("#maincontent table tbody tr th:contains(HomePage)").next()
          .select("a").attr("href");
      String organization = doc.select("#maincontent table tbody tr th:contains(Organization)")
          .next()
          .select("a").text();
      String organizationUrl = doc.select("#maincontent table tbody tr th:contains(Organization)")
          .next()
          .select("a").attr("href");
      ...
      log.ifPresent(l -> l.warn(url + " parse fail", e));
    }
